...
OUTPUT (written to ./output/)
    stock_consolidation_report_india.html   <- open this in a browser, no server needed

EDIT YOUR STOCK LIST
    Scroll down to the "STOCK UNIVERSE" section near the top of this file.
    STOCKS is auto-built from SECTOR_MAP (currently the NSE / Nifty 500
    watchlist, using yfinance-style ".NS" tickers). To change the universe,
    edit SECTOR_MAP — nothing else in the file needs to change.
--------------------------------------------------------------------------
"""
...
# Yahoo Finance has, since 2024, been intermittently rejecting requests that
# don't carry a real browser TLS fingerprint — under concurrent load (our
# ThreadPoolExecutor) this shows up as sporadic, per-symbol "possibly
# delisted; no price data found" / "empty dataframe returned" errors for
# perfectly normal, actively-traded tickers (not an actual delisting).
# curl_cffi's Chrome impersonation avoids this. It's optional: if it isn't
# installed, we fall back to yfinance's default session (still works, just
# more prone to the errors above under heavy concurrency).
...
# report / dashboard). Keys are Yahoo Finance / yfinance-style tickers
# (NSE listings need the ".NS" suffix), values are the sector label.
SECTOR_MAP = {
    # Private Banks
    **{s: "PVT_BANK" for s in [
    ...
    **{s: "PSU_BANK" for s in [
        "SBIN.NS", "BANKBARODA.NS", "CANBK.NS", "UNIONBANK.NS", "PNB.NS",
    ]},
    # Financial Services — NBFCs, insurers, HFCs, capital markets
    **{s: "FIN_SERVICE" for s in [
        "BAJFINANCE.NS", "BAJAJFINSV.NS", "SBILIFE.NS", "HDFCLIFE.NS",
        "SHRIRAMFIN.NS", "MUTHOOTFIN.NS", "CHOLAFIN.NS", "HDFCAMC.NS",
        "PFC.NS", "RECLTD.NS", "IRFC.NS", "TATACAP.NS", "JIOFIN.NS",
    ]},
    # IT & Technology
    **{s: "IT" for s in [
        "INFY.NS", "TCS.NS", "HCLTECH.NS", "TECHM.NS", "WIPRO.NS", "LTM.NS",
    ]},
    # Oil & Gas
    **{s: "OIL&GAS" for s in [
    ...
    **{s: "AUTO" for s in [
        ...
        "MOTHERSON.NS", "BOSCHLTD.NS", "TMCV.NS", "TMPV.NS", "HYUNDAI.NS",
    ]},
    # Pharma & Healthcare
    **{s: "PHARMA" for s in [
        "SUNPHARMA.NS", "CIPLA.NS", "DRREDDY.NS", "DIVISLAB.NS", "APOLLOHOSP.NS",
        "TORNTPHARM.NS", "ZYDUSLIFE.NS", "MAXHEALTH.NS",
    ]},
    # Metals & Mining
    **{s: "METALS" for s in [
        "HINDALCO.NS", "TATASTEEL.NS", "JSWSTEEL.NS", "VEDL.NS", "HINDZINC.NS",
        "JINDALSTEL.NS", "COALINDIA.NS",
    ]},
    # FMCG & Retail
    **{s: "FMCG" for s in [
        "HINDUNILVR.NS", "ITC.NS", "NESTLEIND.NS", "BRITANNIA.NS", "TATACONSUM.NS",
        "GODREJCP.NS", "VBL.NS", "DMART.NS", "UNITDSPR.NS",
    ]},
    # Power & Energy
    **{s: "POWER" for s in [
        "NTPC.NS", "POWERGRID.NS", "TATAPOWER.NS", "ADANIGREEN.NS",
        "ADANIENSOL.NS", "ADANIPOWER.NS",
    ]},
    # Infra, Capital Goods & Defence
    **{s: "INFRA" for s in [
        "LT.NS", "ADANIENT.NS", "ADANIPORTS.NS", "BEL.NS", "HAL.NS", "SIEMENS.NS",
        "ABB.NS", "CGPOWER.NS", "CUMMINSIND.NS", "MAZDOCK.NS", "SOLARINDS.NS",
        "ENRIN.NS",
    ]},
    # Cement
    **{s: "CEMENT" for s in [
    ...
    **{s: "CONS" for s in [
        "TITAN.NS", "TRENT.NS", "ASIANPAINT.NS", "PIDILITIND.NS", "INDHOTEL.NS",
        "DLF.NS", "LODHA.NS",
    ]},
    # Telecom
    **{s: "TELECOM" for s in [
    ...
    # Others / Diversified
    **{s: "OTHER" for s in [
        "BAJAJHLDNG.NS", "ETERNAL.NS", "INDIGO.NS",
    ]},
    # Sector ETFs (BEES)
    **{s: "BEES" for s in [
...
# Readable label for each SECTOR_MAP code, shown in the report's "Index"
# column (next to Company) so it's clear at a glance which sectoral index a
# stock belongs to.
SECTOR_INDEX_LABELS = {
    ...
    "INFRA": "Nifty Infra",
    "CEMENT": "Nifty Cement",
    "CONS": "Nifty Consumer Durables",
    "TELECOM": "Nifty Telecom",
    "OTHER": "Other / Diversified",
    "BEES": "Sector ETF (BEES)",
}
...
COMPANY_NAMES = {
    ...
    "AXISBANK.NS": "Axis Bank Ltd", "KOTAKBANK.NS": "Kotak Mahindra Bank Ltd",
    # --- PSU Banks ---
    "SBIN.NS": "State Bank of India", "BANKBARODA.NS": "Bank of Baroda",
    "CANBK.NS": "Canara Bank", "UNIONBANK.NS": "Union Bank of India",
    "PNB.NS": "Punjab National Bank",
    # --- Financial Services ---
    "BAJFINANCE.NS": "Bajaj Finance Ltd", "BAJAJFINSV.NS": "Bajaj Finserv Ltd",
    "SBILIFE.NS": "SBI Life Insurance Company Ltd", "HDFCLIFE.NS": "HDFC Life Insurance Company Ltd",
    "SHRIRAMFIN.NS": "Shriram Finance Ltd", "MUTHOOTFIN.NS": "Muthoot Finance Ltd",
    "CHOLAFIN.NS": "Cholamandalam Investment and Finance Company Ltd",
    "HDFCAMC.NS": "HDFC Asset Management Company Ltd", "PFC.NS": "Power Finance Corporation Ltd",
    "RECLTD.NS": "REC Ltd", "IRFC.NS": "Indian Railway Finance Corporation Ltd",
    "TATACAP.NS": "Tata Capital Ltd", "JIOFIN.NS": "Jio Financial Services Ltd",
    # --- IT & Technology ---
    "INFY.NS": "Infosys Ltd", "TCS.NS": "Tata Consultancy Services Ltd",
    "HCLTECH.NS": "HCL Technologies Ltd", "TECHM.NS": "Tech Mahindra Ltd",
    "WIPRO.NS": "Wipro Ltd", "LTM.NS": "LTIMindtree Ltd",
    # --- Oil & Gas ---
    "RELIANCE.NS": "Reliance Industries Ltd", "ONGC.NS": "Oil and Natural Gas Corporation Ltd",
    "BPCL.NS": "Bharat Petroleum Corporation Ltd", "GAIL.NS": "GAIL (India) Ltd",
    "IOC.NS": "Indian Oil Corporation Ltd",
    # --- Auto & Auto Ancillary ---
    "MARUTI.NS": "Maruti Suzuki India Ltd", "M&M.NS": "Mahindra & Mahindra Ltd",
    "BAJAJ-AUTO.NS": "Bajaj Auto Ltd", "EICHERMOT.NS": "Eicher Motors Ltd",
    "TVSMOTOR.NS": "TVS Motor Company Ltd", "MOTHERSON.NS": "Samvardhana Motherson International Ltd",
    "BOSCHLTD.NS": "Bosch Ltd", "TMCV.NS": "Tata Motors Commercial Vehicles Ltd",
    "TMPV.NS": "Tata Motors Passenger Vehicles Ltd", "HYUNDAI.NS": "Hyundai Motor India Ltd",
    # --- Pharma & Healthcare ---
    "SUNPHARMA.NS": "Sun Pharmaceutical Industries Ltd", "CIPLA.NS": "Cipla Ltd",
    "DRREDDY.NS": "Dr. Reddy's Laboratories Ltd", "DIVISLAB.NS": "Divi's Laboratories Ltd",
    "APOLLOHOSP.NS": "Apollo Hospitals Enterprise Ltd", "TORNTPHARM.NS": "Torrent Pharmaceuticals Ltd",
    "ZYDUSLIFE.NS": "Zydus Lifesciences Ltd", "MAXHEALTH.NS": "Max Healthcare Institute Ltd",
    # --- Metals & Mining ---
    "HINDALCO.NS": "Hindalco Industries Ltd", "TATASTEEL.NS": "Tata Steel Ltd",
    "JSWSTEEL.NS": "JSW Steel Ltd", "VEDL.NS": "Vedanta Ltd",
    "HINDZINC.NS": "Hindustan Zinc Ltd", "JINDALSTEL.NS": "Jindal Steel & Power Ltd",
    "COALINDIA.NS": "Coal India Ltd",
    # --- FMCG & Retail ---
    "HINDUNILVR.NS": "Hindustan Unilever Ltd", "ITC.NS": "ITC Ltd",
    "NESTLEIND.NS": "Nestle India Ltd", "BRITANNIA.NS": "Britannia Industries Ltd",
    "TATACONSUM.NS": "Tata Consumer Products Ltd", "GODREJCP.NS": "Godrej Consumer Products Ltd",
    "VBL.NS": "Varun Beverages Ltd", "DMART.NS": "Avenue Supermarts Ltd",
    "UNITDSPR.NS": "United Spirits Ltd",
    # --- Power & Energy ---
    "NTPC.NS": "NTPC Ltd", "POWERGRID.NS": "Power Grid Corporation of India Ltd",
    "TATAPOWER.NS": "Tata Power Company Ltd", "ADANIGREEN.NS": "Adani Green Energy Ltd",
    "ADANIENSOL.NS": "Adani Energy Solutions Ltd", "ADANIPOWER.NS": "Adani Power Ltd",
    # --- Infra, Capital Goods & Defence ---
    "LT.NS": "Larsen & Toubro Ltd", "ADANIENT.NS": "Adani Enterprises Ltd",
    "ADANIPORTS.NS": "Adani Ports and Special Economic Zone Ltd", "BEL.NS": "Bharat Electronics Ltd",
    "HAL.NS": "Hindustan Aeronautics Ltd", "SIEMENS.NS": "Siemens Ltd",
    "ABB.NS": "ABB India Ltd", "CGPOWER.NS": "CG Power and Industrial Solutions Ltd",
    "CUMMINSIND.NS": "Cummins India Ltd", "MAZDOCK.NS": "Mazagon Dock Shipbuilders Ltd",
    "SOLARINDS.NS": "Solar Industries India Ltd", "ENRIN.NS": "Siemens Energy India Ltd",
    # --- Cement ---
    "ULTRACEMCO.NS": "UltraTech Cement Ltd", "GRASIM.NS": "Grasim Industries Ltd",
    "AMBUJACEM.NS": "Ambuja Cements Ltd", "SHREECEM.NS": "Shree Cement Ltd",
    # --- Consumer Discretionary ---
    "TITAN.NS": "Titan Company Ltd", "TRENT.NS": "Trent Ltd",
    "ASIANPAINT.NS": "Asian Paints Ltd", "PIDILITIND.NS": "Pidilite Industries Ltd",
    "INDHOTEL.NS": "Indian Hotels Company Ltd", "DLF.NS": "DLF Ltd",
    "LODHA.NS": "Macrotech Developers Ltd",
    # --- Telecom ---
    "BHARTIARTL.NS": "Bharti Airtel Ltd",
    # --- Others / Diversified ---
    "BAJAJHLDNG.NS": "Bajaj Holdings & Investment Ltd", "ETERNAL.NS": "Eternal Ltd",
    "INDIGO.NS": "InterGlobe Aviation Ltd",
    # --- Sector ETFs (BEES) ---
    "NIFTYBEES.NS": "Nippon India ETF Nifty BeES", "BANKBEES.NS": "Nippon India ETF Bank BeES",
    "ITBEES.NS": "Nippon India ETF IT BeES", "AUTOBEES.NS": "Nippon India ETF Auto BeES",
    "PHARMABEES.NS": "Nippon India ETF Pharma BeES", "GOLDBEES.NS": "Nippon India ETF Gold BeES",
...
MAX_WORKERS = 6  # shared pool now handles daily+hourly+15m together (see run_all_scans),
                  ...
                  # if you see a spike in "ERROR" / retry-exhausted rows in the console log,
                  # dial this back down further (e.g. 3-4). Lowered from 12 -> 6 because
                  # Yahoo was intermittently returning empty responses ("possibly
                  # delisted") for perfectly valid, actively-traded NSE tickers under
                  # the old higher concurrency.
REQUEST_RETRIES = 4
...
HTML_REPORT_FILENAME = "stock_consolidation_report_india.html"

# Market label shown in the Telegram summary header (the config file below
# is shared with the USA script, so each script tags its own label).
MARKET_LABEL = "India"
...
# script (shared with consolidation_scanner.py). Environment variables
...
def fetch_data(symbol: str, tf: TimeframeConfig) -> Optional[pd.DataFrame]:
    ...
    for attempt in range(1, REQUEST_RETRIES + 1):
        try:
            ...
            df = ticker.history(period=tf.period, interval=tf.interval, auto_adjust=True)
            ...
            if attempt < REQUEST_RETRIES:
                # Exponential backoff with jitter, so a batch of symbols that
                # all failed together (e.g. a rate-limit blip) doesn't retry
                # in perfect lockstep and immediately re-trigger the limit.
                delay = RETRY_BACKOFF_SECONDS * attempt + random.uniform(0, 1.5)
                time.sleep(delay)
...
<div class="search-wrap">
  <div class="search-box" id="searchBox">
    ...
    <input type="text" id="symbolSearch" placeholder="Search symbol (e.g. INFY)" autocomplete="off" spellcheck="false">
...
# ===========================================================================
# 9b. TELEGRAM SUMMARY (Daily timeframe only — top N per Index)
# ===========================================================================
...
def _display_symbol(symbol: str) -> str:
    """Strip the .NS exchange suffix for the compact Telegram card."""
...
def build_daily_telegram_message(results_by_tf: dict) -> Optional[str]:
    ...
    for idx_label, rows in by_index.items():
        ...
        header = f"{idx_label} ({raw_code})" if raw_code and "(" not in idx_label else idx_label
        lines.append(f"{emoji} {_escape_html(header)}")
        for r in rows:
            icon, short_status = CLASSIFICATION_DISPLAY.get(r.classification or "", ("\u26AA", r.classification or ""))
            lines.append(f"\u2022 #{rank_by_symbol[r.symbol]} {_escape_html(_display_symbol(r.symbol))} \u2014 {r.score} \u2014 {icon} {short_status}")
